  ...
  has_last_control_time_(false)
{
  ...
  // Per-wheel transform tuning.
  // These are intentionally read locally at run time so the existing
  // transform_controller_node.hpp does not need to be changed.
  this->declare_parameter("left_bldc_posvel_kp", 1.0);
  this->declare_parameter("right_bldc_posvel_kp", 1.7);

  this->declare_parameter("left_bldc_posvel_kd", 0.30);
  this->declare_parameter("right_bldc_posvel_kd", 0.15);

  this->declare_parameter("left_bldc_posvel_min_vel_rad_s", 0.4);
  this->declare_parameter("right_bldc_posvel_min_vel_rad_s", 2.0);

  this->declare_parameter("left_bldc_transform_tau_ff", 8.0);
  this->declare_parameter("right_bldc_transform_tau_ff", 1.0);

  // Error-dependent feedforward scheduling.
  this->declare_parameter("bldc_ff_far_error_deg", 15.0);
  this->declare_parameter("bldc_ff_mid_error_deg", 7.0);
  this->declare_parameter("bldc_ff_near_error_deg", 4.0);

  this->declare_parameter("left_bldc_ff_mid_tau", 4.0);
  this->declare_parameter("left_bldc_ff_near_tau", 1.5);

  this->declare_parameter("right_bldc_ff_mid_tau", 0.8);
  this->declare_parameter("right_bldc_ff_near_tau", 0.3);
  ...
  dxl_stop_pub_ = create_publisher<std_msgs::msg::Int32>("/dxl_stop_cmd", 20);

  error_pub_ =
    this->create_publisher<robot_msgs::msg::SystemError>("/system/error", 20);

  pause_sub_ = create_subscription<std_msgs::msg::Bool>("/transform/pause", 20,
    [this](const std_msgs::msg::Bool::SharedPtr msg) {
      if (!msg->data || paused_) { return; }
      paused_ = true;
      paused_at_ = now();
      settle_started_ = false;
      if (step_active_) { stopActiveMotor(); }
    });
  resume_sub_ = create_subscription<std_msgs::msg::Bool>("/transform/resume", 20,
    [this](const std_msgs::msg::Bool::SharedPtr msg) {
      if (!msg->data || !paused_) { return; }
      paused_ = false;
      step_start_time_ = step_start_time_ + (now() - paused_at_);
      resetBldcSpeedLimiter();
      if (!step_active_) { return; }
      if (!feedbackFresh(active_motor_id_)) {
        stopActiveMotor();
        publishStepResult(false, false, "Cannot resume without fresh feedback");
        step_active_ = false;
        active_bldc_posvel_ = false;
        return;
      }
...
void TransformControllerNode::publishBldcSpeedCmd(int motor_id, double v_des)
{
  ...
  // ----------------------------------------------------------
  // Error-dependent transform feedforward scheduling
  //
  // Far from target: strong torque to break static friction.
  // Near target: reduce torque so the wheel can settle instead
  // of repeatedly crossing the target.
  // ----------------------------------------------------------
  double far_tau = 0.0;
  double mid_tau = 0.0;
  double near_tau = 0.0;
  ...
  if (motor_id == 1) {
    this->get_parameter("left_bldc_transform_tau_ff", far_tau);
    this->get_parameter("left_bldc_ff_mid_tau", mid_tau);
    this->get_parameter("left_bldc_ff_near_tau", near_tau);
  } else if (motor_id == 2) {
    this->get_parameter("right_bldc_transform_tau_ff", far_tau);
    this->get_parameter("right_bldc_ff_mid_tau", mid_tau);
    this->get_parameter("right_bldc_ff_near_tau", near_tau);
  }

  far_tau = std::abs(far_tau);
  mid_tau = std::abs(mid_tau);
  near_tau = std::abs(near_tau);

  double far_error_deg = 15.0;
  double mid_error_deg = 7.0;
  double near_error_deg = 4.0;

  this->get_parameter("bldc_ff_far_error_deg", far_error_deg);
  this->get_parameter("bldc_ff_mid_error_deg", mid_error_deg);
  this->get_parameter("bldc_ff_near_error_deg", near_error_deg);

  far_error_deg = std::abs(far_error_deg);
  mid_error_deg = std::abs(mid_error_deg);
  near_error_deg = std::abs(near_error_deg);

  // Keep the thresholds ordered even if YAML is edited incorrectly.
  if (mid_error_deg < near_error_deg) {
    mid_error_deg = near_error_deg;
  }
  if (far_error_deg < mid_error_deg) {
    far_error_deg = mid_error_deg;
  }

  const double current_angle_deg = getMotorAngleDeg(motor_id);
  const double error_deg = shortestWrappedErrorDeg(
    current_angle_deg,
    target_angle_deg_,
    bldc_wrap_range_deg_);

  const double abs_error_deg = std::abs(error_deg);

  double scheduled_tau = 0.0;

  if (abs_error_deg > far_error_deg) {
    scheduled_tau = far_tau;
  } else if (abs_error_deg > mid_error_deg) {
    scheduled_tau = mid_tau;
  } else if (abs_error_deg > near_error_deg) {
    scheduled_tau = near_tau;
  } else {
    scheduled_tau = 0.0;
  }

  // Stop commands must never keep feedforward torque applied.
  if (std::abs(v_des) < 1e-6) {
    msg.tau_ff = 0.0;
  } else {
    msg.tau_ff = (v_des > 0.0)
      ? scheduled_tau
      : -scheduled_tau;
  }
  ...
  RCLCPP_INFO_THROTTLE(
    ...
    *this->get_clock(),
    500,
    "BLDC MIT speed cmd motor=%d err=%+.3f deg v_des=%+.3f scheduled_tau=%+.3f",
    motor_id,
    error_deg,
    v_des,
    msg.tau_ff);
  ...
  const robot_msgs::msg::TransformStep::SharedPtr msg)
{
  ...
  if (!std::isfinite(msg->target_angle_deg) || !std::isfinite(timeout_sec_) ||
    timeout_sec_ <= 0 || !feedbackFresh(active_motor_id_) ||
    (active_motor_type_ == MOTOR_TYPE_BLDC && active_motor_id_ > 2) ||
    (active_motor_type_ == MOTOR_TYPE_DXL && active_motor_id_ < 3))
  {
    publishStepResult(false, false, "Invalid step or missing/stale feedback");
    return;
  }
  ...
  if (active_motor_type_ == MOTOR_TYPE_BLDC) {
    ...
    if (!paused_) { publishDxlPositionCmd(active_motor_id_, target_angle_deg_); }
...
bool TransformControllerNode::feedbackFresh(int motor_id) const
{
  const std::string names[] = {motor1_joint_name_, motor2_joint_name_,
    motor3_joint_name_, motor4_joint_name_};
  if (motor_id < 1 || motor_id > 4) { return false; }
  const auto it = joint_feedback_time_.find(names[motor_id - 1]);
  if (it == joint_feedback_time_.end()) { return false; }
  const double age = (now() - it->second).seconds();
  return age >= 0.0 && age <= get_parameter("feedback_timeout_sec").as_double() &&
    std::isfinite(getMotorAngleDeg(motor_id)) &&
    std::isfinite(getMotorVelocityRad(motor_id));
}

void TransformControllerNode::stopActiveMotor()
{
  if (active_motor_type_ == MOTOR_TYPE_BLDC) {
    publishBldcStopCmd();
  } else if (active_motor_type_ == MOTOR_TYPE_DXL) {
    // The hardware stop topic captures PRESENT_POSITION directly on the bus;
    // no stale ROS angle is used as a replacement target.
    std_msgs::msg::Int32 msg;
    msg.data = active_motor_id_;
    dxl_stop_pub_->publish(msg);
  }
}
  ...
  const sensor_msgs::msg::JointState::SharedPtr msg)
{
  for (std::size_t i = 0; i < msg->name.size(); ++i) {
    if (i >= msg->position.size() || i >= msg->velocity.size() ||
      !std::isfinite(msg->position[i]) || !std::isfinite(msg->velocity[i])) { continue; }
...
void TransformControllerNode::dxlJointStateCallback(
  const sensor_msgs::msg::JointState::SharedPtr msg)
{
  bldcJointStateCallback(msg);
}

void TransformControllerNode::controlTimerCallback()
{
  if (!step_active_ || paused_) {
    return;
  }
  ...
  if (!feedbackFresh(active_motor_id_)) {
    stopActiveMotor();
    publishStepResult(false, false, "Motor feedback expired");
    step_active_ = false;
    active_bldc_posvel_ = false;
    return;
  }
  ...
  if (active_motor_type_ == MOTOR_TYPE_BLDC && active_bldc_posvel_) {
    ...
    // Per-wheel transform tuning
    // ----------------------------------------------------------
    double active_kp = bldc_posvel_kp_;
    double active_kd = bldc_posvel_kd_;
    double active_min_vel = bldc_posvel_min_vel_rad_s_;

    if (active_motor_id_ == 1) {
      this->get_parameter("left_bldc_posvel_kp", active_kp);
      this->get_parameter("left_bldc_posvel_kd", active_kd);
      this->get_parameter("left_bldc_posvel_min_vel_rad_s", active_min_vel);
    } else if (active_motor_id_ == 2) {
      this->get_parameter("right_bldc_posvel_kp", active_kp);
      this->get_parameter("right_bldc_posvel_kd", active_kd);
      this->get_parameter("right_bldc_posvel_min_vel_rad_s", active_min_vel);
    }

    active_kp = std::max(0.0, active_kp);
    active_kd = std::max(0.0, active_kd);
    active_min_vel = std::max(0.0, active_min_vel);

    double desired_v_cmd =
      active_kp * error_rad
      - active_kd * current_vel_rad;
    ...
    if (std::abs(error_deg) > bldc_posvel_position_tolerance_deg_ &&
        std::abs(desired_v_cmd) < active_min_vel) {
    ...
    RCLCPP_INFO_THROTTLE(
      ...
      *this->get_clock(),
      ...
      "v_fb=%.3f desired_v=%.3f kp=%.3f kd=%.3f min_v=%.3f pos_ok=%d vel_ok=%d",
      active_motor_id_,
      current_angle_deg_raw,
      target_angle_deg_,
      error_deg,
    ...
    // Extended-position DXL goals are signed and continuous, not modulo 360/1080.
